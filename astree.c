...
PROCFUNCDEF *make_procfuncdef (DECLLIST *optfp, TYPE *rt, BLOCK *blk)
{
  PROCFUNCDEF *p = anew (PROCFUNCDEF);
  p->proc_formals = optfp;
  p->proc_result_type = rt;
  p->proc_block = blk;
  p->proc_syminfo = 0;
...
DECL *make_procfunc_decl (IDENT *id, DECLLIST *optfp, TYPE *rt, BLOCK *blk)
{
  DECL *d = make_decl (DeclProcFunc_, id);
  d->d.proc = make_procfuncdef (optfp, rt, blk);
  return d;
}
...
TYPE *make_pointer_type (IDENT *id)
{
  TYPE *t = make_type (TypePointer_);
  t->t.ptr = make_id_type(id);
  return t;
}
/* origin */
...
/* FieldList constructors */
FIELD * make_field(IDENT *id, TYPE *t, int offset)
{
  FIELD *f = anew(FIELD);
  f->field_id = id;
  f->field_type = t;
...
FIELDLIST *make_fieldlist(IDENT *id, TYPE *t, FIELDLIST *optfl)
{
  FIELDLIST *fl = anew(FIELDLIST);
  fl->this = make_field(id, t, 0);
  fl->rest = optfl;
  return fl;
}
/**********************************************************************/
...
STMTREPEAT *make_stmtrepeat (STMTLIST *sl, EXPR *e)
{
...
STMTLIST *make_stmtlist (STMT *s, STMTLIST *rest)
{
  if(s == 0){
	return rest;
  }
...
STMTIF *make_stmtif (EXPR *if_expr, STMT *if_then, STMT *if_else)
{
...
STMTFOR *make_stmtfor (IDENT *id, EXPR* ini, int dir, EXPR *to, STMT *s)
{
  ...
  f->for_to = to;
  f->for_stmt = s;
  f->for_upward = ( dir == TO_ ? TRUE : FALSE);
  f->for_limit_obj = 0;
  return f;
}
...
STMT *make_if_stmt (EXPR *if_expr, STMT *if_then, STMT *if_else)
{
  STMT *s = make_stmt (StmtIf_);
  s->s.ifx = make_stmtif (if_expr, if_then, if_else);
  return s;
}

STMT *make_case_stmt (EXPR *expr, CASELIST *casle)
{
  STMT *s = make_stmt (StmtCase_);
  s->s.cas = make_stmtcase (expr, casle);
  return s;
}

CASELIST *make_caselist (CASEARM *casle, CASELIST *optcasles)
{
  if( casle == 0){
	return optcasles;
  }
  else{
    CASELIST *casl = anew(CASELIST);
    casl->this = casle;
    casl->rest = optcasles;
    return casl;
  }
...
CASEARM *make_casearm (EXPRLIST *cas_labels, STMT *s)
{
  CASEARM *casa = anew(CASEARM);
  casa->arm_labels = cas_labels;
  casa->arm_stmt = s;
  return casa;
}

STMT *make_repeat_stmt (STMTLIST *sl, EXPR *e)
{
  STMT *s = make_stmt (StmtRepeat_);
  s->s.rpt = make_stmtrepeat (sl, e);
  return s;
}

STMT *make_for_stmt (IDENT *id, EXPR* ini, int dir, EXPR *to, STMT *st)
{
  STMT *s = make_stmt (StmtFor_);
  s->s.forx = make_stmtfor (id, ini, dir, to, st);
  return s;
}
